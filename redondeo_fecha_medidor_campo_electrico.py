    ...
    for n_linea in range(len(negativo)):
        if negativo.iloc[n_linea, 0] == fecha:
            if n_linea == len(negativo)-1:
                new_archivo = new_archivo.append({'Fecha':fecha, 'Polaridad':negativo.iloc[n_linea, 1], 'NivelCE':st.mean(moda_array), 'FalloRotor':negativo.iloc[n_linea, 3]}, ignore_index=True)
            else:
                moda_array.append(negativo.iloc[n_linea, 2])
        else:
            print(st.mode(moda_array))
            new_archivo = new_archivo.append({'Fecha':fecha, 'Polaridad':negativo.iloc[n_linea-1, 1], 'NivelCE':st.mean(moda_array), 'FalloRotor':negativo.iloc[n_linea-1, 3]}, ignore_index=True)
